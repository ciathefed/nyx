...
fn substituteStatement(self: *Preprocessor, stmt: ast.Statement, param_map: *std.AutoHashMap(StringId, *ast.Expression)) !?ast.Statement {
    ...
    return switch (stmt) {
        ...
        .@"extern" => |v| .{ .@"extern" = .{ .name = try self.substituteExprWithParams(v.name, param_map), .param_types = v.param_types, .return_type = v.return_type, .span = v.span } },
...
fn processStatement(self: *Preprocessor, stmt: ast.Statement) !?ast.Statement {
    ...
    return switch (stmt) {
        ...
        .@"extern" => |v| .{ .@"extern" = .{ .name = try self.substituteExpr(v.name), .param_types = v.param_types, .return_type = v.return_type, .span = v.span } },
